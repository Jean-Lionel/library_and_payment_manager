...
<div class="col-md-8 offset-md-2">
    ...
    {{-- <div class="form-group">
        <label for="titulaire">Enseignant titulaire</label>
        <select class="form-control" name="titulaire">
            @if ($titulaires->count() > 0)
            @foreach ($titulaires as $titu)
            <option value="{{$titu->id}}">{{$titu->role_id}}</option>
            @endforeach
            @endif
        </select>
    </div> --}}
